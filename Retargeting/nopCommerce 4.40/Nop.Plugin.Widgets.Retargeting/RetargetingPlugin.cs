        ...
        public async Task<string> ExportProductsToCsvAsync()
        {
            ...
            foreach (var product in products)
            {
                ...
                foreach (var currentProduct in productsToProcess)
                {
                    ...
                    var categories = await (await _categoryService.GetProductCategoriesByProductIdAsync(currentProduct.Id))
                        .SelectAwait(async x => await _categoryService.GetCategoryByIdAsync(x.CategoryId))
                        .ToListAsync();
                    //extra data
                    //categories (breadcrumb)
                    var categoryString = new Dictionary<string, object>{};
                    ...
                    {
                       categoryString.Add("\"Root\"","\"Default category\"");
                    }
